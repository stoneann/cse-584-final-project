...
public class StreamJoin {
    ...
    public static void main(String[] args) {
        ...
        int partitionBegin = Integer.parseInt(args[5]);
        int partitionEnd = Integer.parseInt(args[6]);
        for (int i = partitionBegin; i < partitionEnd; i++) {
            used_partitions.add(i);
            filesToRead.add(args[0] + i + ".csv");
        }

        List<String> filesToReadSmall = new ArrayList<String>();

        int partitionBeginSmall = Integer.parseInt(args[7]);
        int partitionEndSmall = Integer.parseInt(args[8]);
        for (int i = partitionBeginSmall; i < partitionEndSmall; i++) {
            filesToReadSmall.add(args[0] + i + ".csv");
        }

        var arg4Tokens = args[4].split(",");
        List<String> fileNames = new ArrayList<String>();
        for (String token : arg4Tokens) {
            fileNames.add(token);
        }
        ...
        Dataset<Row> df2 = spark.read()
                .option("header", "false")
                .csv(filesToReadSmall.toArray(new String[filesToRead.size()]))
                .select("_c0", "_c1", "_c2")
        ...
        var ans = df2.mapPartitions((MapPartitionsFunction<Row, Row>) s -> {
                    ...
                    try {
                        for(int i = 0; i < fileNames.size(); i++){
                            System.err.println("S3 File: " + String.format(indexPath + "sm_source_file=%s/", smallTableName) + fileNames.get(i));
                            S3Object s3Object = s3Client.getObject("584spark-east2", String.format(indexPath + "sm_source_file=%s/", smallTableName) + fileNames.get(i));
